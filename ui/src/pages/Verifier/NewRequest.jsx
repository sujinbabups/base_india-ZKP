...
localStorage.setItem('category',selectedOption);
  ...
  return (
	...
	<div className='lg:w-[400px] lg:h-[670px] md:w-[400px] md:h-[450px] w-[350px] h-[470px] border-[3px] border-white rounded-lg'>
	  <p>Connected Account: {account}</p>
  
	  <div className='text-center mt-5 text-2xl font-bold text-white'>Request Form</div>
	  ...
	  <form className='flex flex-col gap-4 text-black lg:ml-[20%] ml-[15%] mt-[10%]' onSubmit={handleSubmit}>
		<div>
		  ...
		  <label className="block text-sm font-medium text-white mt-4">Select an Option</label>
		  <select
			id="selectOption"
			...
			className="mt-1 block w-[250px] p-2 bg-gray-50 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
		  >
			<option value="choose">Choose what to verify</option>
		...
		{selectedOption === 'age' && (
		  <div className="mt-4">
			...
			<input
			  ...
			  className="mt-1 block w-[250px] p-2 bg-gray-50 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
			/>
			<label className="block text-sm font-medium text-white mt-4">Maximum Age</label>
			<input
			  id="maxAgeInput"
			  type="text"
		...
		<button
		  type="submit"
		  className='w-[250px] bg-[orange] h-10 text-center rounded-lg text-white font-semibold mt-6'
		>
		  Send Request
		</button>
	  </form>
